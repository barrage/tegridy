 ...
 * <p>
 * The single `modify` method of this interface should never have to be implemented manually
 * as it is intended to be used in conjunction with Modify annotations.
 * <p>
 * Its default implementation will reflect on the class and apply any modifiers specified through annotations.
 * <p>
 * Annotations for modifying strings are readily available. In special cases where one needs specific/custom
 * modification the `ModifyCustom` annotation can be used. Additionally, any nested classes can also be
 * modified from a single call from the parent by annotating them with `ModifyNested`.
 */
public interface Modify {

  static final Map<Class<?>, Class<?>> MODIFIERS = new HashMap<>() {{
