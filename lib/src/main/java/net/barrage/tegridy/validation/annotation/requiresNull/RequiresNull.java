...
/**
 * Defines a validation constraint that enforces certain fields to be null if a specific field is present.
 * This annotation is intended for use on classes to establish validation rules that ensure if a specific condition
 * is met (specific field is present), then other specified fields must be null.
 *
 * <p>This annotation is repeatable, allowing multiple conditional null validations to be applied to a class.</p>
 * <p>
 * Usage example:
 * <pre>
 ...
 * <p>
 * Attributes:<br>
 * - field: Specifies the name of the field that triggers the validation. If this field is present, the specified forbiddenFields must be null.<br>
 * - forbiddenFields: Defines the fields that are required to be null if the triggering field is present.<br>
 ...
 *
 * @see RequiresNullValidator The validator implementing the constraint logic, ensuring the specified conditions are met.
 */
...
public @interface RequiresNull {
  ...
  /**
   * Specifies the field that, if present, enforces the specified {@code forbiddenFields} to be null.
   *
