...
public class CompareProcessor extends AbstractProcessor {
  ...
  public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
    roundEnv
        .getElementsAnnotatedWithAny(Set.of(CompareList.class, Compare.class))
        .forEach(
            annotatedElement -> {
    ...
    if (!elementHasField(annotatedElement, baseField)
      ...
      processingEnv
          ...
          .printMessage(
              ...
              String.format(
                  "The class '%s' is missing required fields by @Compare: '%s', '%s'.",
            ...
            () ->
                processingEnv
                    ...
                    .printMessage(
                        Diagnostic.Kind.ERROR,
                        String.format(
                            "Method '%s' required by @Compare not found in class '%s'.",
                            comparisonMethod, annotatedElement.getSimpleName()),
                        annotatedElement));
  }

  private void validateMethod(
      ExecutableElement method,
      TypeMirror baseFieldType,
      TypeMirror comparisonFieldType,
      Types typeUtils,
    ...
    if (!isBooleanType) {
      processingEnv
          ...
          .printMessage(
              Diagnostic.Kind.ERROR,
              String.format(
                  "Method '%s' in class '%s' must return boolean as required by @Compare.",
                  comparisonMethod, annotatedElement.getSimpleName()),
              annotatedElement);
      return;
    }
