...
/**
 * Defines a validation constraint that enforces certain fields to be non-null if a specific field
 * is present and not null. This annotation is intended for use on classes to establish conditional
 * validation rules between fields, ensuring that if a certain condition is met (specific field is
 * not null), then other specified fields must also be not null.
 *
 * <p>This annotation is repeatable, allowing multiple conditional non-null validations to be
 * applied to a class. Usage example:
 *
 * <pre>
 ...
 * - requiresFields: Defines the fields that are required to be not null if the triggering field is
 * present and not null.<br>
...
public @interface RequiresNotNull {
  ...
  /**
   * Specifies the field that, if not null, triggers validation of the specified {@code
   * requiresFields}.
   *
